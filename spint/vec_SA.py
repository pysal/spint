...
class VecMoran:
    ...
    def __init__(
        self, y, w, focus="origin", rand="A", permutations=PERMUTATIONS, two_tailed=True
    ):
        self.y = y
        self.o = y[:, 1:3]
        self.d = y[:, 3:5]
    ...
    def __moments(self):
        ...
        d = (n - 1) * (n - 2) * (n - 3)
        self.VI_rand = 1 / (W**2 * m2**2) * ((n1 - n2) / d + (n3 - n4) / d) + (
            (a2 * b2) - m2**2
        ) / (m2**2 * (n - 1) ** 2)
        self.seI_rand = self.VI_rand ** (1 / 2.0)
